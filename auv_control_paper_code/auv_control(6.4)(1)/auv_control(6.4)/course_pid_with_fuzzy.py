...
class CoursePID:
    ...
    # 更新PID控制器状态
    def update(self, feedback_course, feedback_dvl_x, feedback_roll):
        ...
        if delta_time >= self.sample_time:                # 如果超过采样时间
            if feedback_dvl_x > 1:
                ...
                self.steer_ang = 1.5 * feedback_roll  # 定向同时打舵
            else:
                # 加入了模糊Kp
                self.PTerm = self.fuzzy_res_kp * self.Kp * error
            if abs(error) <= 20:                            # 如果误差<10度
