...
class RollPID:
    def __init__(self, Kp, Ki, Kd, sample_time=0.00):
        ...
        self.current_roll = 0.0     # 当前横滚角
        self.exp_roll = 0.0  # 目标位置
        self.PTerm = 0.0  # 比例项
        self.ITerm = 0.0  # 积分项
        self.DTerm = 0.0  # 微分项
    ...
    def clear(self):
        ...
        self.DTerm = 0.0         # 微分项
        self.last_error = 0.0    # 上一次误差
        self.windup_guard = 20   # 防止积分值过大（积分饱和保护）
        self.output = 0.0        # PID输出值

    # 更新PID控制器状态
    def update(self, feedback_roll, feedback_dvl_x):
        # error = self.exp_roll - feedback_roll  # 计算误差
        error =feedback_roll - self.exp_roll  # ！！！使用当前roll-期望roll，
        # 使得PID输出为正值，最后使用正负输出给舵角
        # 偏转角最小化处理
        # if error > 180:
        #     error = error - 360
        # elif error < -180:
        ...
        if delta_time >= self.sample_time:                # 如果超过采样时间
            if feedback_dvl_x > 1:
                self.PTerm = 2 * self.Kp * error * (1/feedback_dvl_x)  # 计算比例项
            else:
                self.PTerm = self.Kp * error
            # self.PTerm = self.Kp * error
            if abs(error) <= 5:                             # 如果误差<10度
    ...
    # 设置目标位置
    def set_roll(self, exp_roll):
        self.exp_roll = exp_roll
